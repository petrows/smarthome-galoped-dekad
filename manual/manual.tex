...
\item Power Supply: 5V USB-C; \\ \textbf{Only USB-A-to-C cable is supported}
...
\textbf{tasmota-xxxx-nnnn} (where \textbf{xxxx} are the last 4 digits of the MAC address).
...
the web interface, please refer to the official Tasmota documentation at \texttt{https://tasmota.github.io/docs/} .
...
If WiFi settings are not configured, device will switch off Access Point after 5 minutes. To restart configuration
process, please restart device by unplugging power.

\section{Support and Troubleshooting}

If you have issues with device, please visit GitHub project page for support:

\medskip

\texttt{https://github.com/petrows/smarthome-galoped-dekad}

\medskip

Device is fully open-source and can be modified easily. Project page provides all
files to compile and modify the firmware.

\medskip

If device does not power on: Device supports the \textbf{USB-A to C} connection for power supply only.
